...
const AnecdoteList = ({ anecdotes }) => (
  <div>
    ...
    <ul>
      {anecdotes
        .map(anecdote => <li key={anecdote.id} ><Link to={`/anecdote/${anecdote.id}`}>{anecdote.content}</Link></li>)}
    </ul>
  </div>
)

const Anecdote = ({ anecdote }) => (<div>{anecdote.content}</div>)
...
const CreateNew = (props) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    props.notification(`Added ${content.value}`);
    props.addNew({
  ...
  return (
    <div>
      ...
      <Switch>
        ...
        <Route path="/create">
          <CreateNew notification={manageNotification} addNew={addNew} />
        </Route>
      </Switch>
